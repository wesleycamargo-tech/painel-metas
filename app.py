...
try:
    ...
    # Varre a planilha linha por linha mapeando a estrutura vertical real
    for idx, row in df_raw.iterrows():
        if len(row) < 2:
            continue
            
        # Coleta os dados da Coluna A (index 0) e da Coluna B (index 1)
        col_a = str(row.iloc[0]).strip().lower() if pd.notna(row.iloc[0]) else ""
        col_b = str(row.iloc[1]).strip() if pd.notna(row.iloc[1]) else ""
        
        # Controla dinamicamente os blocos de competências empilhados
        if "julho" in col_a:
            current_section = "Julho / 2026"
        elif "junho" in col_a:
            current_section = "Junho / 2026"
        elif "maio" in col_a:
            current_section = "Maio / 2026"
            
        # Se a linha pertence ao mês selecionado no filtro lateral
        if current_section == competencia:
            if col_b != "" and col_b != "nan":
                col_b_upper = col_b.upper()
                # Ignora títulos complementares e labels de agrupamento
                if "METAS" in col_b_upper or "INDICADOR" in col_b_upper or "PONDERAÇÃO" in col_b_upper or "PESO" in col_b_upper or "FAIXA" in col_b_upper:
                    continue
                    
                # Mapeia as metas diretamente pelas posições fixas das colunas (C=2, D=3, E=4, F=5)
                linhas_validas.append({
                    "INDICADOR": col_b,
                    "RE": str(row.iloc[2]).strip() if len(row) > 2 and pd.notna(row.iloc[2]) else "-",
                    "CSF INTERNO": str(row.iloc[3]).strip() if len(row) > 3 and pd.notna(row.iloc[3]) else "-",
                    "CSF AJUDA": str(row.iloc[4]).strip() if len(row) > 4 and pd.notna(row.iloc[4]) else "-",
                    "CSF QUALITY": str(row.iloc[5]).strip() if len(row) > 5 and pd.notna(row.iloc[5]) else "-"
                })

    if len(linhas_validas) == 0:
        st.warning(f"Nenhum indicador mapeado para a competência '{competencia}'.")
    else:
        # ==============================================================================
        # QUADRO 1: MATRIZ DE INDICADORES (CONSTRUÍDA DINAMICAMENTE)
        # ==============================================================================
        ...
        for item in linhas_validas:
            ...
            for cluster in clusters_filtrados:
                meta_val = item[cluster]
                
                # Atribuição dos pesos regulamentares fixos por indicador
                peso_val = "-"
